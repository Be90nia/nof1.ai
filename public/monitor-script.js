...
class TradingMonitor {
    ...
    try {
      ...
      if (unrealisedPnlEl) {
        ...
        unrealisedPnlEl.className =
          "detail-value " + (data.unrealisedPnl >= 0 ? "positive" : "negative");
      }
      ...
      if (valueChangeEl && valuePercentEl) {
        ...
        valueChangeEl.textContent = `${isPositive ? "+" : ""}$${Math.abs(
          totalPnl
        ).toFixed(2)}`;
        valuePercentEl.textContent = `(${
          isPositive ? "+" : ""
        }${returnPercent.toFixed(2)}%)`;

        // 更新颜色
        valueChangeEl.className =
          "change-amount " + (isPositive ? "" : "negative");
        valuePercentEl.className =
          "change-percent " + (isPositive ? "" : "negative");
      }
    } catch (error) {
  ...
  async loadStrategyData() {
    try {
      ...
      // 更新策略详细信息（一行显示）
      const strategyInfoInline = document.getElementById(
  ...
  async loadPositionsData() {
    try {
      ...
      const positionsCardsContainer = document.getElementById(
      ...
      if (!data.positions || data.positions.length === 0) {
        ...
        if (positionsBody) {
          positionsBody.innerHTML =
            '<tr><td colspan="9" class="empty-state">暂无持仓</td></tr>';
        }
        // 更新小卡片
        if (positionsCardsContainer) {
          positionsCardsContainer.innerHTML =
      ...
      // 更新持仓表格
      if (positionsBody) {
        positionsBody.innerHTML = data.positions
          .map((pos) => {
            const profitPercent = (
              (pos.unrealizedPnl / pos.openValue) *
              100
            ).toFixed(2);
      ...
      // 更新持仓小卡片
      if (positionsCardsContainer) {
        positionsCardsContainer.innerHTML = data.positions
          .map((pos) => {
            const profitPercent = (
              (pos.unrealizedPnl / pos.openValue) *
              100
            ...
            return `
                        <div class="position-card ${sideClass} ${pnlClass}">
                            <span class="position-card-symbol">${
                              pos.symbol
                            } ${leverage}x</span>
      ...
      if (!data.trades || data.trades.length === 0) {
        if (tradesBody) {
          tradesBody.innerHTML =
            '<tr><td colspan="9" class="empty-state">暂无交易记录</td></tr>';
        }
        if (countEl) {
          countEl.textContent = "";
        }
  ...
  async loadTickerPrices() {
    try {
      const response = await fetch(
        "/api/prices?symbols=BTC,ETH,SOL,BNB,DOGE,XRP"
      );
      const data = await response.json();
  ...
  startDataUpdates() {
    ...
    // 每30秒更新交易记录和日志
    setInterval(async () => {
      await Promise.all([this.loadTradesData(), this.loadLogsData()]);
    }, 30000);
    ...
    if (!historyData || historyData.length === 0) {
      ...
      if (container) {
        ...
        message.style.cssText =
          "position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%); color: #00cc88; text-align: center;";
        message.innerHTML =
          '暂无历史数据<br><small style="color: #008866;">系统将每10分钟自动记录账户资产</small>';
