  ...
  try {
    ...
    if (dbPositions.rows.length > 0) {
      for (const row of dbPositions.rows) {
        ...
        console.log(
          `      方向: ${pos.side === "long" ? "做多 (LONG)" : "做空 (SHORT)"}`
        );
        console.log(`      数量: ${pos.quantity} 张`);
        console.log(
          `      开仓价: ${Number.parseFloat(pos.entry_price).toFixed(4)}`
        );
        console.log(
          `      当前价: ${Number.parseFloat(pos.current_price).toFixed(4)}`
        );
        console.log(`      杠杆: ${pos.leverage}x`);
        console.log(
          `      未实现盈亏: ${
            pos.unrealized_pnl >= 0 ? "+" : ""
          }${Number.parseFloat(pos.unrealized_pnl).toFixed(2)} USDT`
        );
    ...
    const dbSymbols = new Set(dbPositions.rows.map((row: any) => row.symbol));

    // 检查 Gate.io 有但数据库没有的
    const missingInDb = Array.from(gateSymbols).filter(
      (s) => !dbSymbols.has(s)
    );
    if (missingInDb.length > 0) {
      console.log(
        `   ⚠️ Gate.io 有但数据库缺失的持仓: ${missingInDb.join(", ")}`
      );
    }

    // 检查数据库有但 Gate.io 没有的
    const missingInGate = Array.from(dbSymbols).filter(
      (s) => !gateSymbols.has(s)
    );
    if (missingInGate.length > 0) {
      console.log(
        `   ⚠️ 数据库有但 Gate.io 已平仓的持仓: ${missingInGate.join(", ")}`
      );
    }

    // 检查两边都有的，对比详细信息
    const commonSymbols = Array.from(gateSymbols).filter((s) =>
      dbSymbols.has(s)
    );
    for (const symbol of commonSymbols) {
      ...
      if (gateSide !== dbPos.side) {
        inconsistencies.push(`方向不一致: Gate=${gateSide}, DB=${dbPos.side}`);
      }
      ...
      for (const row of trades.rows) {
        ...
        const pnl = trade.pnl
          ? ` | 盈亏: ${
            ...
            }${Number.parseFloat(trade.pnl).toFixed(2)} USDT`
          : "";

        console.log(`   📝 ${trade.timestamp}`);
        console.log(`      ${type} ${trade.symbol} ${side}`);
        console.log(
          `      价格: ${Number.parseFloat(trade.price).toFixed(4)} | 数量: ${
            trade.quantity
          } 张 | 杠杆: ${trade.leverage}x${pnl}`
        );
        console.log(
          `      订单ID: ${trade.order_id} | 状态: ${trade.status}\n`
        );
      }
    } else {
    ...
    if (closeTrades.rows.length > 0) {
      ...
      for (const row of closeTrades.rows) {
        ...
        const pnl = trade.pnl
          ...
          : "0.00";
        console.log(
          `      ${trade.symbol}: side=${trade.side} (原持仓方向), 数量=${trade.quantity}张, 盈亏=${pnl}U`
        );
      }
      console.log();
    }
    ...
    console.log("📝 说明:");
    console.log(
      "   - trades表中的 side 字段表示持仓方向（long=做多, short=做空）"
    );
    console.log(
      "   - 开仓记录: side=持仓方向，实际执行=long时买入(+size)，short时卖出(-size)"
    );
    console.log(
      "   - 平仓记录: side=原持仓方向，实际执行=long时卖出(-size)，short时买入(+size)"
    );
