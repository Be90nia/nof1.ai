						...
						if (peakPnlResult.rows.length > 0) {
							...
							if (peakPnlPercent > 0) {
								...
								if (
									...
									let trendAgainstUs = false; // 趋势是否对我们不利
									...
									try {
										...
										if (klines.length >= 10) {
											...
											const previousVolume =
												volumes.slice(-10, -3).reduce((sum, v) => sum + v, 0) /
												7;
											...
											if (isNowLosing) {
												shouldTrigger = true;
												indicators.push("🔴 当前已亏损，必须执行峰值回落保护");
												logger.warn(
											...
											// 🎯 Level1额外判断：如果不满足容忍条件，默认触发
												...
												shouldTrigger = true;
												indicators.push("⚠️ Level1严格模式：不满足容忍条件，执行全平");
												logger.warn(
										...
										if (isNowLosing) {
											shouldTrigger = true;
											logger.warn(
												`${symbol} 当前已亏损，即使无法获取指标数据，也应执行峰值回落保护`,
											);
										}
									}
									...
									if (shouldTrigger) {
										...
										const initialQuantity = dbInfo?.initial_quantity || currentSize;
