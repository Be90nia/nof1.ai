...
export class CaiSenBatchClosingSystem extends EventEmitter {
      ...
      switch (triggerCondition.triggerType) {
        case "price":
          return this.compareValues(
            Number.parseFloat(position.markPrice || "0"),
  ...
  private checkReturnPredictionCondition(
    ...
    if (aiPrediction.recommendedStrategy === "batch_close") {
  ...
  private async checkMarketSentimentCondition(
    ...
    try {
      ...
      if (marketSentiment.fearAndGreedIndex < 30) {
    ...
    try {
          ...
          if (orderFilled) {
            ...
            if (orderDetails) {
              ...
              if (feeRate > 0 && actualPrice > 0 && actualExecutedQty > 0) {
                actualFee =
                  actualPrice * actualExecutedQty * quantoMultiplier * feeRate;
              }
      ...
      try {
        ...
        if (actualFee === 0) {
          ...
          const closeFee = Math.abs(
            actualPrice * actualExecutedQty * quantoMultiplier * 0.0005
          );
          actualFee = closeFee;
      ...
      try {
        ...
        // 🔧 参数验证：确保所有值都有效
        const orderId = closeResult.id?.toString() || `batch_${Date.now()}`;
        ...
        const type =
          batchState.config.closingType === ClosingType.TAKE_PROFIT
            ...
            ? "stop_loss"
            : "risk_mitigation";
        const price = batchState.executionResult.actualPrice ?? 0;
        const quantity = batchState.executionResult.actualQuantity ?? 0;
        const leverage = Number.parseFloat(position.leverage || "1") || 1;
        const pnl = batchState.executionResult.pnl ?? 0;
        const fee = batchState.executionResult.fee ?? 0;
        const timestamp = new Date().toISOString();
        const status = "filled";

        // 🔧 验证所有参数都不是 undefined 或 null
        if (
          orderId === undefined ||
          orderId === null ||
          symbol === undefined ||
          ...
          throw new Error(
            `数据库参数验证失败: orderId=${orderId}, symbol=${symbol}, side=${side}, type=${type}, price=${price}, quantity=${quantity}, leverage=${leverage}, pnl=${pnl}, fee=${fee}, timestamp=${timestamp}, status=${status}`
          );
        }
        ...
        // 🔧 新增：记录平仓决策到 agent_decisions 表
        try {
          const recordClosingModule = await import(
            "../monitor/recordClosingDecision"
          );
          const { recordClosingDecision, ClosingTriggerType } =
            recordClosingModule;
          const { iterationCount } = await import(
            "../../../scheduler/tradingLoop"
          );

          // 确定触发类型
          let triggerType: (typeof ClosingTriggerType)[keyof typeof ClosingTriggerType];
          if (batchState.config.closingType === ClosingType.TAKE_PROFIT) {
            triggerType = ClosingTriggerType.PARTIAL_TAKE_PROFIT;
          } else if (batchState.config.closingType === ClosingType.STOP_LOSS) {
            triggerType = ClosingTriggerType.DYNAMIC_STOP_LOSS;
          } else {
            triggerType = ClosingTriggerType.PARTIAL_TAKE_PROFIT; // 默认
          }

          // 计算盈亏百分比
          const entryPriceNum =
            position.entry_price ||
            Number.parseFloat(position.entryPrice || "0");
          const leverage = Number.parseFloat(position.leverage || "1") || 1;
          const priceChangePercent =
            entryPriceNum > 0
              ? ((actualPrice - entryPriceNum) / entryPriceNum) * 100
              : 0;
          const pnlPercent =
            position.side === "long"
          ...
          // 计算平仓比例
          const totalQuantity = position.positionAmt || actualExecutedQty;
          const closePercent = (actualExecutedQty / totalQuantity) * 100;
          ...
          await recordClosingDecision(
            {
              ...
              triggerType,
              level: batchState.config.batchId,
              entryPrice: entryPriceNum,
              currentPrice: actualPrice,
              closePrice: actualPrice,
              leverage,
              pnlPercent,
              triggerThreshold: 0, // 批次平仓没有固定阈值
              currentValue: pnlPercent,
              closePercent,
              closeQuantity: actualExecutedQty,
              totalQuantity,
              pnl: batchState.executionResult.pnl,
              fee: batchState.executionResult.fee,
              description: "批次平仓系统触发",
            },
            iterationCount
          );
          ...
          logger.info({
            ...
            symbol,
            triggerType,
            message: "平仓决策已记录到 agent_decisions 表",
          });
        } catch (decisionError) {
          logger.warn({
