...
export class CaiSenBatchClosingSystem extends EventEmitter {
	...
	private async executeBatchInternal(batchState: BatchState): Promise<void> {
		try {
			...
			// 使用 Big.js 精确计算平仓数量
			const positionAmtBig = new Big(position.positionAmt);
			const closingRatioBig = new Big(batchState.config.closingRatio);
			const configQuantityBig = new Big(batchState.config.closingQuantity);

			let actualQuantityBig: Big;
			if (configQuantityBig.gt(0)) {
				// 如果指定了数量，使用指定数量，但不超过实际持仓
				actualQuantityBig = configQuantityBig.gt(positionAmtBig.abs())
					? positionAmtBig.abs()
					: configQuantityBig;
			} else {
				// 如果未指定数量，按比例计算
				actualQuantityBig = positionAmtBig.abs().mul(closingRatioBig);
			}

			// 确保平仓数量为正数
			if (actualQuantityBig.lte(0)) {
				throw new Error(
					`Invalid closing quantity: ${actualQuantityBig.toString()}`,
				);
			}
			...
			const closeResult = await this.exchangeClient.closePosition({
				...
				size: Number.parseFloat(actualQuantityBig.toString()),
			});

			// 更新执行结果 - Update execution result
			batchState.executionResult = {
				actualQuantity: Number.parseFloat(
					(closeResult.executedQty || actualQuantityBig).toString(),
				),
				actualPrice: Number.parseFloat(
					(closeResult.avgPrice || position.markPrice).toString(),
				),
				fee: Number.parseFloat((closeResult.fee || 0).toString()),
				pnl: Number.parseFloat((closeResult.realizedPnl || 0).toString()),
			};
