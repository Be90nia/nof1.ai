...
export class CaiSenBatchClosingInstructionRecognizer extends EventEmitter {
    ...
    try {
      ...
      switch (instruction.type) {
        ...
        case InstructionType.MODIFY_BATCH_PARAMS:
          isValid = await this.validateModifyBatchParamsInstruction(
            instruction
          );
          break;

        case InstructionType.CANCEL_BATCH_CLOSING:
          isValid = await this.validateCancelBatchClosingInstruction(
            instruction
          );
          break;
  ...
  async executeInstruction(instructionId: string): Promise<boolean> {
    try {
      ...
      switch (instruction.type) {
        ...
        case InstructionType.EMERGENCY_CLOSING:
          isSuccess = await this.executeEmergencyClosingInstruction(
            instruction
          );
          break;

        case InstructionType.MODIFY_BATCH_PARAMS:
          isSuccess = await this.executeModifyBatchParamsInstruction(
            instruction
          );
          break;

        case InstructionType.CANCEL_BATCH_CLOSING:
          isSuccess = await this.executeCancelBatchClosingInstruction(
            instruction
          );
          break;
  ...
  private async executeBatchClosingInstruction(
    ...
    try {
      ...
      const success = await this.batchClosingSystem.setAiClosingParams(
        aiParams
      );
