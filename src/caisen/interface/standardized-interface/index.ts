...
import { InterfaceCallResult, type InterfaceCallResponse, type BatchClosingParameters, type StopProfitLossParameters } from "../types";
import {
...
export class CaiSenStandardizedInterface extends EventEmitter {
   ...
   */
  async setBatchClosing(parameters: BatchClosingParameters): Promise<InterfaceCallResponse<string>> {
   ...
   * @returns Promise<InterfaceCallResponse<string>> 调用结果 - Call result
   */
  async setStopProfitLossThreshold(parameters: StopProfitLossParameters): Promise<InterfaceCallResponse<string>> {
    const startTime = Date.now();

    try {
      // 验证参数 - Validate parameters
      const validationResult = this.validateStopProfitLossParameters(parameters);
      if (!validationResult.valid) {
        return {
          result: InterfaceCallResult.PARAM_ERROR,
          errorMessage: validationResult.errorMessage,
   ...
   */
  async getPositionManagementStatus(positionId: string): Promise<InterfaceCallResponse<AllStatusInfo>> {
    const startTime = Date.now();
   ...
   */
  private validateBatchClosingParameters(parameters: BatchClosingParameters): { valid: boolean; errorMessage?: string } {
    if (!parameters.positionId) {
      return { valid: false, errorMessage: "Position ID is required" };
    }

    if (!parameters.batchPercentages || parameters.batchPercentages.length === 0) {
      return { valid: false, errorMessage: "Batch percentages are required" };
    }

    const totalPercentage = parameters.batchPercentages.reduce((sum: number, percentage: number) => sum + percentage, 0);
    if (Math.abs(totalPercentage - 100) > 0.01) {
      return { valid: false, errorMessage: "Total batch percentages must equal 100%" };
    }

    if (!parameters.triggerConditions || parameters.triggerConditions.length === 0) {
   ...
   * @returns { valid: boolean; errorMessage?: string } 验证结果 - Validation result
   */
  private validateStopProfitLossParameters(parameters: StopProfitLossParameters): { valid: boolean; errorMessage?: string } {
    if (!parameters.positionId) {
      return { valid: false, errorMessage: "Position ID is required" };
    }
