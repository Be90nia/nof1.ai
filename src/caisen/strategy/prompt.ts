		...
		for (const pos of positions) {
			...
			if (hasAddedPosition) {
				...
				prompt +=
					...
					" 张 (增加了 " +
					(
						((currentQuantity - initialQuantity) / initialQuantity) *
						100
					).toFixed(2) +
					"%)\n";
				...
				if (pos.last_add_position_time) {
					...
					prompt +=
						"  ✓ 上次加仓时间: " +
						formatChinaTime(pos.last_add_position_time) +
						" (距今 " +
						minutesSinceLastAdd +
						" 分钟)\n";
				}
			...
			if (hasAddedPosition) {
				prompt +=
					"  平均成本: " + effectiveEntryPrice.toFixed(2) + " USDT (已加仓)\n";
			} else {
