...
export function generateCaiSenPrompt(
	...
	for (const [symbol, dataRaw] of Object.entries(marketData)) {
		...
		prompt += `\n${symbol} 数据\n`;
		// 核心指标（所有提到的指标都包含）
		prompt += `当前: 价格=${data.price.toFixed(2)}, EMA10=${data.ema10?.toFixed(3) || data.ema20.toFixed(3)}, EMA20=${data.ema20.toFixed(3)}, MACD=${data.macd.toFixed(3)}, RSI7=${data.rsi7.toFixed(2)}, RSI14=${data.rsi14?.toFixed(2) || "N/A"}\n`;
		prompt += `指标: MFI=${data.mfi.toFixed(2)}, StochK=${data.stochasticK.toFixed(2)}, StochD=${data.stochasticD.toFixed(2)}, ADX=${data.adx.toFixed(2)}\n`;
		prompt += `波动率: ATR3=${data.atr3.toFixed(2)}, ATR14=${data.atr14.toFixed(2)}, 布林上=${data.bollingerUpper.toFixed(2)}, 布林中=${data.bollingerMiddle.toFixed(2)}, 布林下=${data.bollingerLower.toFixed(2)}\n`;
		prompt += `成交量: ${data.volume.toFixed(2)}, OBV=${data.obv.toFixed(0)}, VWAP=${data.vwap.toFixed(2)}, 恐惧贪婪=${data.fearAndGreedIndex.toFixed(2)}\n`;
		...
		if (data.microstructure) {
			...
			let microstructureText = `微观结构: 订单簿不平衡度=${Number(ms.orderBookMetrics.orderBookImbalance).toFixed(4)}, 买卖价差=${Number(ms.orderBookMetrics.spread).toFixed(4)}\n`;
			...
			// 检查是否有实际有意义的数值要显示
			const hasBidDepth =
				!Number.isNaN(bidDepthChangeRate) && formattedBidDepth !== "0.00";
			const hasAskDepth =
				!Number.isNaN(askDepthChangeRate) && formattedAskDepth !== "0.00";
			...
			// 检查是否有实际有意义的数值要显示
			if (!Number.isNaN(buySellRatio) && formattedBuySell !== "0.00") {
			...
			const hasExecutionSpeed =
				!Number.isNaN(executionSpeed) && formattedExecutionSpeed !== "0.00";
			const hasLiquidityRatio =
				!Number.isNaN(liquidityRatio) && formattedLiquidityRatio !== "0.00";
			...
			// 检查是否有实际有意义的数值要显示
			const hasVwap = !Number.isNaN(vwap) && formattedVwap !== "0.00";
			const hasBidSlope =
				!Number.isNaN(bidSlope) && formattedBidSlope !== "0.00";
			const hasAskSlope =
				!Number.isNaN(askSlope) && formattedAskSlope !== "0.00";
			const hasPriceImpact =
				!Number.isNaN(priceImpact) && formattedPriceImpact !== "0.0000";
		...
		if (data.intradaySeries && data.intradaySeries.midPrices.length > 0) {
			...
			prompt += `5分钟序列（最近${recentCount}个）: 价格=[${recentPrices.map((p: number) => p.toFixed(2)).join(", ")}]`;

			if (recentRsi7.length > 0) {
				prompt += `, RSI7=[${recentRsi7.map((r: number) => r.toFixed(1)).join(", ")}]`;
			}
			if (recentMacd.length > 0) {
				prompt += `, MACD=[${recentMacd.map((m: number) => m.toFixed(2)).join(", ")}]`;
			}
			prompt += "\n";
		}
		...
		if (data.timeframes) {
			...
			for (const tf of keyTimeframes) {
				...
				if (tfData) {
					prompt += `${tf.name}: 价格=${tfData.currentPrice.toFixed(2)}, EMA10=${tfData.ema10?.toFixed(3) || tfData.ema20.toFixed(3)}, RSI7=${tfData.rsi7.toFixed(2)}, MACD=${tfData.macd.toFixed(3)}, 成交量=${tfData.volume.toFixed(2)}\n`;
				}
			}
		}
	}
	...
	if (positions.length > 0) {
		...
		prompt +=
			"- ⚠️ 【加仓后重要提醒】：加仓后平均成本已改变，所有盈亏计算都基于新的平均成本，必须重新设置退出策略！\n\n";
		...
		for (const pos of positions) {
			...
			if (hasAddedPosition) {
				...
				const totalAddAmount = pos.total_add_amount_usdt || 0;
				const initialQuantity = pos.initial_quantity || pos.quantity;
				const currentQuantity = pos.quantity;

				prompt += "\n🔥 【加仓状态 - 重要】\n";
				prompt += `  ✓ 已执行加仓: ${addPositionCount} 次\n`;
				prompt += `  ✓ 初始开仓价格: ${pos.entry_price.toFixed(2)} USDT\n`;
				prompt += `  ✓ 当前平均成本: ${effectiveEntryPrice.toFixed(2)} USDT (通过加仓降低了 ${costReduction.toFixed(2)}%)\n`;
				prompt += `  ✓ 初始持仓数量: ${initialQuantity.toFixed(4)} 张\n`;
				prompt += `  ✓ 当前持仓数量: ${currentQuantity.toFixed(4)} 张 (增加了 ${(
					((currentQuantity - initialQuantity) / initialQuantity) * 100
				).toFixed(2)}%)\n`;
				prompt += `  ✓ 累计加仓金额: ${totalAddAmount.toFixed(2)} USDT\n`;
				...
				if (pos.last_add_position_time) {
					...
					prompt += `  ✓ 上次加仓时间: ${formatChinaTime(pos.last_add_position_time)} (距今 ${minutesSinceLastAdd} 分钟)\n`;
				}

				prompt += `\n  ⚠️ 【关键提醒】：加仓后平均成本已从 ${pos.entry_price.toFixed(2)} 降至 ${effectiveEntryPrice.toFixed(2)}，\n`;
				prompt += `     所有盈亏计算、止盈止损阈值都必须基于新的平均成本 ${effectiveEntryPrice.toFixed(2)} 重新计算！\n`;
				prompt +=
					"     必须立即调用 setPositionExitStrategy 工具重新设置退出策略参数！\n\n";
			}

			// 基本信息
			prompt += "【基本信息】\n";
			prompt += `  杠杆倍数: ${pos.leverage}x\n`;
			prompt += `  当前市场价: ${pos.current_price.toFixed(2)} USDT\n`;
			if (hasAddedPosition) {
				prompt += `  平均成本: ${effectiveEntryPrice.toFixed(2)} USDT (已加仓)\n`;
			} else {
				prompt += `  开仓价格: ${effectiveEntryPrice.toFixed(2)} USDT\n`;
			}
			...
			if (pnlPercent < 0 && addPositionCount < 3) {
				...
				if (priceDropPercent >= 3 && priceDropPercent <= 15) {
					prompt += `  💡 加仓机会: 当前亏损 ${Math.abs(pnlPercent).toFixed(2)}%，价格下跌 ${priceDropPercent.toFixed(2)}%\n`;
					prompt += `  💡 加仓进度: ${addPositionCount}/3 次\n`;
					...
					if (pos.last_add_position_time) {
						...
						if (minutesSinceLastAdd >= 30) {
							prompt += `  ✓ 时间间隔: ${minutesSinceLastAdd} 分钟 (≥30分钟，满足要求)\n`;
						} else {
							prompt += `  ✗ 时间间隔: ${minutesSinceLastAdd} 分钟 (<30分钟，需等待 ${30 - minutesSinceLastAdd} 分钟)\n`;
						}
					} else {
						prompt += "  ✓ 时间间隔: 首次加仓，无时间限制\n";
					}

					prompt +=
						"  💡 决策建议: 请综合评估趋势方向、支撑位、成交量、风险回报比等因素\n";
				} else if (priceDropPercent < 3) {
					prompt += `  ℹ️  加仓条件: 价格下跌 ${priceDropPercent.toFixed(2)}% < 3%，未达到加仓阈值\n`;
				} else if (priceDropPercent > 15) {
					prompt += `  ⚠️  风险警告: 价格下跌 ${priceDropPercent.toFixed(2)}% > 15%，风险过高，应考虑止损而非加仓\n`;
				}
			} else if (addPositionCount >= 3) {
				prompt +=
					"\n【加仓限制】\n  ⚠️  已达到最大加仓次数（3次），无法继续加仓\n";
			}
