...
export function getSwingTrendStrategy(maxLeverage: number): StrategyParams {
	return {
		...
		// ==================== 策略规则描述 ====================
		entryCondition:
			"必须1分钟、3分钟、5分钟、15分钟这4个时间框架信号全部强烈一致，且关键指标共振（MACD、RSI、EMA方向一致）", // 入场条件（严格）
		riskTolerance: "单笔交易风险控制在20-35%之间，注重趋势质量而非交易频率", // 风险容忍度
		tradingStyle:
			"波段趋势交易，20分钟执行周期，耐心等待高质量趋势信号，持仓时间可达数天，让利润充分奔跑", // 交易风格
 ...
 */
export function generateSwingTrendPrompt(
