...
export function getCaiSenStrategy(maxLeverage: number): StrategyParams {
  ...
  return {
    // ==================== 策略基本信息 ====================
    name: "蔡森策略",
    description: "多时间框架分析+七分位策略引擎+动态点位交易系统，精准捕捉买卖点和暴跌后反弹机会，适合稳健投资者",
    ...
    // ==================== 策略规则描述 ====================
    entryCondition: "多时间框架分析确认+七分位策略信号，暴跌后在1/7或1/2区域考虑做多，突破高点考虑追涨",
    riskTolerance: "单笔交易风险最大1%账户资金，单日最大损失5%，动态止损+分批止盈",
    tradingStyle: "多维度分析+精准买卖点+暴跌反弹策略，中等频率交易，注重风险控制",
 ...
 */
export function generateCaiSenPrompt(params: StrategyParams, context: StrategyPromptContext): string {
  // 计算杠杆推荐值（用于提示词）
