 ...
 */
export function getMultiAgentConsensusStrategy(
	maxLeverage: number,
): StrategyParams {
	// 计算策略的杠杆范围：使用 55%-80% 的最大杠杆
	const levMin = Math.max(2, Math.ceil(maxLeverage * 0.55)); // 最小杠杆：55%最大杠杆，至少2倍
	const levMax = Math.max(3, Math.ceil(maxLeverage * 0.8)); // 最大杠杆：80%最大杠杆，至少3倍
	...
	return {
		...
		name: "陪审团策略",
		description:
			"法官与陪审团合议决策，主Agent独立分析+三个专业Agent辅助，追求高质量决策",
 ...
 */
export function generateMultiAgentConsensusPrompt(
