...
import {
	...
	stopStopLossMonitor,
} from "./scheduler/stopLossMonitor";
import { initTradingSystem, startTradingLoop } from "./scheduler/tradingLoop";
import {
	startTrailingStopMonitor,
	stopTrailingStopMonitor,
} from "./scheduler/trailingStopMonitor";
...
async function main() {
	...
	// 判断是否为双重防护模式
	const isDualProtection =
		isCodeLevelEnabled && params.allowAiOverrideProtection === true;
	const protectionMode = isDualProtection
		? " (🛡️ 双重防护: 代码自动 + AI主动)"
		: isCodeLevelEnabled
			? " (启用代码级保护)"
			: " (AI主导控制)";
	...
	if (isCodeLevelEnabled) {
		...
		logger.info(
			`  • Stage 2: 盈利达到 ${params.partialTakeProfit.stage2.trigger}% 时，平仓 ${params.partialTakeProfit.stage2.closePercent}%`,
		);
		logger.info(
			`  • Stage 3: 盈利达到 ${params.partialTakeProfit.stage3.trigger}% 时，平仓 ${params.partialTakeProfit.stage3.closePercent}%`,
		);
	...
	logger.info(
		`🟢 账户止盈线: ${process.env.ACCOUNT_TAKE_PROFIT_USDT || 10000} USDT (触发后全部清仓并退出)`,
	);
	logger.info("\n按 Ctrl+C 停止系统\n");
}
