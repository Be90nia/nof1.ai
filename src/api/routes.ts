...
export function createApiRoutes() {
	...
	app.get("/api/trades", async (c) => {
		try {
			...
			const trades = result.rows.map((row: any) => {
				...
				const contractQuantity = row.quantity
	...
	app.post("/api/close-position", async (c) => {
		try {
			...
			if (currentPrice <= 0) {
				try {
					...
					if (marketPrice > 0) {
						...
						logger.error(
							`无法获取有效的市场价格，markPrice: ${gatePosition.markPrice}, ticker.last: ${ticker.last}`,
						);
						// 尝试使用入场价作为最后的备选方案
						if (entryPrice > 0) {
			...
			logger.info(
				`已下达手动平仓订单 ${symbol}，订单ID: ${order.id}，预期平仓数量: ${quantity}`,
			);
				...
				try {
					...
					if (orderInfo.status === "finished") {
						...
						if (fillPrice > 0) {
							...
							logger.warn(
								`订单 ${order.id} 的价格信息无效，使用当前价格: ${currentPrice}`,
							);
						}
						...
						if (filledSize > 0) {
							...
							logger.info(
								`订单 ${order.id} 实际成交数量: ${actualCloseQuantity}`,
							);
						}
					...
					logger.warn(
						`获取订单信息失败: ${error.message}，使用当前价格: ${currentPrice}`,
					);
					actualExitPrice = currentPrice;
				}
			} else {
			...
			if (actualExitPrice <= 0) {
				...
				if (entryPrice > 0) {
					...
					logger.error(
						`所有价格获取都失败，使用入场价作为平仓价格的最后备选: ${actualExitPrice}`,
					);
				} else {
			...
			if (remainingQuantity > 0.0001) {
				// 设置一个小的阈值，避免浮点数精度问题
				logger.warn(
					`订单未完全成交，剩余数量: ${remainingQuantity}，尝试平掉剩余仓位`,
				);
				...
				try {
					...
					if (updatedPosition) {
						...
						if (updatedQuantity > 0.0001) {
							// 尝试平掉剩余仓位
							const remainingCloseSize =
								side === "long" ? -updatedQuantity : updatedQuantity;
							...
							logger.info(
								`已下达剩余仓位平仓订单，订单ID: ${remainingOrder.id}，数量: ${updatedQuantity}`,
							);
								...
								try {
									...
									if (remainingOrderInfo.status === "finished") {
										...
										if (remainingFilledSize > 0) {
											...
											logger.info(
												`剩余订单实际成交数量: ${Math.abs(remainingFilledSize)}，总成交数量: ${actualCloseQuantity}`,
											);
										}
									}
								} catch (error: any) {
