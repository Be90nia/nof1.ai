...
export function fixJsonFormat(jsonString: string): string {
  ...
  try {
    ...
    let fixedJson = jsonrepair(jsonString);
      ...
      if (
        ...
        const extractPeakDrawdown = () => {
          ...
          const peakDrawdownRegex =
            /"peakDrawdown"\s*:\s*\{[^}]*\{[^}]*\}[^}]*\}/g;
          let matches = Array.from(fixedJson.matchAll(peakDrawdownRegex));
          if (matches.length > 0) {
            const component = matches[matches.length - 1][0];
            components.push(component);
...
function extractToolCallFromText(
  ...
  while ((toolCallBlockMatch = toolCallBlocksRegex.exec(text)) !== null) {
    ...
    if (toolNameMatch) {
      ...
      if (thresholdMatch) {
        params.threshold = parseFloat(thresholdMatch[1]);
      }
      ...
      if (intervalMatch) {
        params.evaluationInterval = parseInt(intervalMatch[1]);
      }
      ...
      if (toolName === "setPartialTakeProfitParams") {
        ...
        if (stage1Match) {
          ...
          params.stage1 = {
            trigger: triggerMatch ? parseFloat(triggerMatch[1]) : 0,
            closePercent: closePercentMatch
              ? parseFloat(closePercentMatch[1])
              : 0,
          };
        }
      ...
      // 处理 setPeakDrawdownParams 特殊参数
        ...
        if (level1Match) {
          ...
          params.level1 = {
            drawdownThreshold: drawdownMatch ? parseFloat(drawdownMatch[1]) : 0,
            closePercent: closePercentMatch
              ? parseFloat(closePercentMatch[1])
              : 0,
          };
        ...
        if (minHoldingTimeMatch) {
          params.minHoldingTime = parseInt(minHoldingTimeMatch[1]);
        }
      }
      // 处理 setPositionExitStrategy 特殊参数
        ...
        if (peakDrawdownMatch && peakDrawdownMatch[1].trim() !== "未配置") {
          ...
          if (minHoldingTimeMatch) {
            params.peakDrawdown.minHoldingTime = parseInt(
        ...
        if (
          ...
          if (initialStopLossMatch) {
            params.dynamicStopLoss.initialStopLoss = parseFloat(
          ...
          if (trailingStopLossMatch) {
            params.dynamicStopLoss.trailingStopLoss = {
              level1: {
                trigger: parseFloat(trailingStopLossMatch[1]),
                stopAt: parseFloat(trailingStopLossMatch[2]),
              },
            };
          }
          ...
          if (trailingStopLoss2Match) {
            ...
            params.dynamicStopLoss.trailingStopLoss.level2 = {
              trigger: parseFloat(trailingStopLoss2Match[1]),
              stopAt: parseFloat(trailingStopLoss2Match[2]),
            };
          }
          ...
          if (trailingStopLoss3Match) {
            ...
            params.dynamicStopLoss.trailingStopLoss.level3 = {
              trigger: parseFloat(trailingStopLoss3Match[1]),
              stopAt: parseFloat(trailingStopLoss3Match[2]),
            };
          }
        }
      }
