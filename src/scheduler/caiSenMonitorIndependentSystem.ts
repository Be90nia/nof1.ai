...
import { 
  ...
  GapPeriodTakeoverParameters,
  OpeningMonitoringAssociationParameters
} from './caiSenStandardizedInterface';
...
export interface CaiSenMonitorConfig {
  ...
  monitoringTypes: ('stop_loss' | 'take_profit' | 'trailing_stop' | 'partial_profit')[];
...
export class CaiSenMonitorIndependentSystem extends EventEmitter {
  ...
  async createMonitor(
    ...
    try {
      ...
      if (this.monitors.has(config.monitorId)) {
        return {
          result: InterfaceCallResult.ALREADY_EXISTS,
  ...
  async startMonitor(
    ...
    try {
      ...
      const positionInfoResult = await this.standardizedInterface.getPositionInfo(monitor.positionId);
      ...
      monitorState.thresholds = this.dynamicThresholdSetting.getThresholdsByPositionId(monitor.positionId);
      
      // 获取相关分批平仓 - Get related batch closings
      monitorState.batchClosings = this.batchClosingSystem.getBatchStatesByPositionId(monitor.positionId);
      
      // 获取空窗期接管状态 - Get gap period takeover state
      const takeoverStates = this.gapPeriodTakeover.getTakeoverStatesByPositionId(monitor.positionId);
      ...
      if (monitorState) {
        ...
        monitorState.lastError = error instanceof Error ? error.message : 'Unknown error';
  ...
  async stopMonitor(
    ...
    try {
      ...
      // 检查监控器状态 - Check monitor status
      if (monitorState.status === CaiSenMonitorStatus.STOPPED || 
  ...
  async deleteMonitor(
    ...
    try {
      ...
      if (monitor.enabled) {
        ...
        if (stopResult.result !== InterfaceCallResult.SUCCESS && 
            stopResult.result !== InterfaceCallResult.ALREADY_EXISTS) {
          return stopResult;
        }
      }
   ...
   * getAllMonitorStates()
   */
  async getAllMonitorStates(): Promise<InterfaceCallResponse<CaiSenMonitorState[]>> {
    const startTime = Date.now();
  ...
  private async checkThresholds(
    ...
    const thresholds = this.dynamicThresholdSetting.getThresholdsByPositionId(monitor.positionId);
    ...
    for (const threshold of thresholds) {
      ...
      // 检查阈值是否触发 - Check if threshold is triggered
        const triggered = await this.dynamicThresholdSetting.checkThresholdTrigger(threshold.thresholdId);
  ...
  private async checkBatchClosings(
    ...
    const batchClosings = this.batchClosingSystem.getBatchStatesByPositionId(monitor.positionId);
    ...
    for (const batchClosing of batchClosings) {
      ...
      const triggered = await this.batchClosingSystem.checkBatchClosing(batchClosing.config.batchId);
  ...
  private async checkGapPeriodTakeover(
    ...
    if (takeoverStates.length > 0) {
      ...
      const triggered = this.gapPeriodTakeover.checkTakeoverTriggered(takeoverState.takeoverId);
  ...
  private async executeThresholdAction(
    ...
    try {
      ...
      logger.error(`为监控器 ${monitorId} 执行阈值操作时出错`, { error, threshold });
    }
  }
  ...
  private async executeBatchClosingAction(
    ...
    try {
      ...
      const executed = await this.batchClosingSystem.executeBatch(batchClosing.config.batchId);
      
      if (executed) {
        logger.info(`Batch closing executed for monitor ${monitorId}`, { batchId: batchClosing.config.batchId });
      } else {
        logger.warn(`Failed to execute batch closing for monitor ${monitorId}`, { batchId: batchClosing.config.batchId });
      }
    } catch (error) {
      logger.error(`为监控器 ${monitorId} 执行分批平仓操作时出错`, { error, batchClosing });
    }
  }
  ...
  private async executeGapPeriodTakeoverAction(
    ...
    try {
      ...
      const executed = this.gapPeriodTakeover.executeTakeover(takeoverState.takeoverId);
      
      if (executed) {
        logger.info(`Gap period takeover executed for monitor ${monitorId}`, { takeoverId: takeoverState.takeoverId });
      } else {
        logger.warn(`Failed to execute gap period takeover for monitor ${monitorId}`, { takeoverId: takeoverState.takeoverId });
      }
    } catch (error) {
      logger.error(`为监控器 ${monitorId} 执行空窗期接管操作时出错`, { error, takeoverState });
    }
  }
  ...
  private async executeTakeProfit(
    ...
    logger.info(`Executing take profit for monitor ${monitorId}`, { threshold });
  }
  ...
  private async executeTrailingStop(
    ...
    logger.info(`Executing trailing stop for monitor ${monitorId}`, { threshold });
  }
  ...
  private async executePartialProfit(
    ...
    logger.info(`Executing partial profit for monitor ${monitorId}`, { threshold });
  }
  ...
  private setupInterSystemEventListening(): void {
    ...
    this.batchClosingInstructionRecognizer.on('instructionRecognized', (data) => {
      // 转发事件 - Forward event
  ...
  private validateMonitorConfig(
    ...
    if (!config.direction || !['long', 'short'].includes(config.direction)) {
      return { valid: false, errorMessage: 'Direction must be either "long" or "short"' };
    }
    
    if (!config.monitoringTypes || config.monitoringTypes.length === 0) {
      return { valid: false, errorMessage: 'Monitoring types must be specified' };
    }
    
    if (!config.monitoringInterval || config.monitoringInterval <= 0) {
      return { valid: false, errorMessage: 'Monitoring interval must be greater than 0' };
    }
    
    if (!config.dataSource || !['exchange', 'database', 'cache', 'custom'].includes(config.dataSource)) {
      return { valid: false, errorMessage: 'Data source must be one of "exchange", "database", "cache", or "custom"' };
    }
    
    if (config.dataSource === 'custom' && !config.customDataSource) {
      return { valid: false, errorMessage: 'Custom data source function is required when data source is "custom"' };
    }
