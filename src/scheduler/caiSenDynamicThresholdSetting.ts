...
import { CaiSenBatchClosingSystem, ClosingType, BatchConfig } from './caiSenBatchClosingSystem';
import { CaiSenAiParameterControl, ClosingParameterDetail } from './caiSenAiParameterControl';
...
export class CaiSenDynamicThresholdSetting extends EventEmitter {
   ...
   */
  updateDynamicThreshold(thresholdId: string, updates: Partial<DynamicThreshold>): boolean {
    try {
      ...
      const updatedThreshold = { ...threshold, ...updates, updatedAt: Date.now() };
    ...
    try {
      ...
      if (threshold.type === ThresholdType.STOP_LOSS) {
        ...
        if (threshold.direction === 'long' && threshold.currentPrice <= calculatedValue) {
          isTriggered = true;
        } else if (threshold.direction === 'short' && threshold.currentPrice >= calculatedValue) {
          isTriggered = true;
        }
      } else if (threshold.type === ThresholdType.TAKE_PROFIT) {
   ...
   * @returns DynamicThreshold[] 阈值列表 - List of thresholds
   */
  getThresholdsByType(type: ThresholdType, status?: ThresholdStatus): DynamicThreshold[] {
   ...
   */
  getAllThresholds(type?: ThresholdType, status?: ThresholdStatus): DynamicThreshold[] {
  ...
  private parseThresholdData(thresholdData: any): any | null {
    try {
      ...
      if (!thresholdData.type || !thresholdData.positionId || !thresholdData.symbol || 
          !thresholdData.direction || !thresholdData.entryPrice) {
   ...
   * @private
   */
  private calculateAtrThreshold(threshold: DynamicThreshold, marketData?: any): number {
   ...
   * @private
   */
  private calculateBollingerThreshold(threshold: DynamicThreshold, marketData?: any): number {
   ...
   * @private
   */
  private calculateFibonacciThreshold(threshold: DynamicThreshold, marketData?: any): number {
   ...
   * @private
   */
  private calculatePivotThreshold(threshold: DynamicThreshold, marketData?: any): number {
   ...
   * @private
   */
  private calculateCustomThreshold(threshold: DynamicThreshold, marketData?: any): number {
    ...
    try {
      ...
      for (const threshold of activeThresholds) {
        if (threshold.expiresAt && threshold.expiresAt < now) {
          ...
          this.updateDynamicThreshold(threshold.thresholdId, { status: ThresholdStatus.EXPIRED });
          
          // 发出阈值过期事件 - Emit threshold expiration event
          this.emit('thresholdExpired', { thresholdId: threshold.thresholdId, threshold });
   ...
   * @private
   */
  private async applyUpdateRule(threshold: DynamicThreshold, rule: UpdateRule): Promise<void> {
    try {
      switch (rule.type) {
        case 'time_based':
