...
export interface BatchState {
  ...
  /** 当前状态 - Current status */
  status: BatchStatus.PENDING | BatchStatus.EXECUTING | BatchStatus.COMPLETED | BatchStatus.FAILED | BatchStatus.CANCELLED;
...
export class CaiSenBatchClosingSystem extends EventEmitter {
  ...
  private async checkAndExecuteBatches(): Promise<void> {
    try {
      ...
      const executingBatches = Array.from(this.batchStates.values())
        .filter(batch => batch.status === BatchStatus.EXECUTING);
      
      const availableSlots = this.config.maxConcurrentBatches - executingBatches.length;
   ...
   */
  private async checkTriggerCondition(batchState: BatchState): Promise<boolean> {
    try {
      ...
      const position = await this.exchangeClient.getPositions().then(positions => 
  ...
  private async executeBatchInternal(batchState: BatchState): Promise<void> {
    try {
      ...
      this.emit('batchExecutionStarted', { batchId: batchState.config.batchId });
  ...
  async executeBatch(batchId: string): Promise<boolean> {
    try {
      ...
      if (batchState.status !== BatchStatus.PENDING && batchState.status !== BatchStatus.EXECUTING) {
      ...
      const currentStatus = batchState.status as BatchStatus.PENDING | BatchStatus.EXECUTING | BatchStatus.COMPLETED | BatchStatus.FAILED | BatchStatus.CANCELLED;
      return currentStatus === BatchStatus.COMPLETED || currentStatus === BatchStatus.FAILED;
    } catch (error) {
  ...
  public async startBatchClosing(positionId: string): Promise<boolean> {
    try {
      ...
      for (const [batchId, batchState] of this.batchStates.entries()) {
        if (batchState.config.positionId === positionId && batchState.status === BatchStatus.PENDING) {
      ...
      logger.info(`Started batch closing for position: ${positionId}`, { batchCount: relatedBatches.length });
      return true;
    } catch (error) {
  ...
  public async stopBatchClosing(positionId: string): Promise<boolean> {
    try {
      ...
      for (const [batchId, batchState] of this.batchStates.entries()) {
        if (batchState.config.positionId === positionId && 
            (batchState.status === BatchStatus.PENDING || batchState.status === BatchStatus.EXECUTING)) {
      ...
      logger.info(`Stopped batch closing for position: ${positionId}`, { batchCount: relatedBatches.length });
      return true;
    } catch (error) {
  ...
  cancelBatchClosing(batchId: string): boolean {
    try {
      ...
      if (batchState.status === BatchStatus.COMPLETED || batchState.status === BatchStatus.CANCELLED) {
        logger.warn('Batch is already completed or cancelled', { batchId, status: batchState.status });
        return false;
      }
  ...
  getBatchStatesByPositionId(positionId: string): BatchState[] {
    return Array.from(this.batchStates.values())
      .filter(batch => batch.config.positionId === positionId);
  }
...
export function createCaiSenBatchClosingSystem(
  ...
  return new CaiSenBatchClosingSystem(
    config,
    strategyConfig
  );
}