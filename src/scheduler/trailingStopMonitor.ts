...
function checkExitConditions(
	...
	if (params.partialTakeProfit) {
		...
		// 🔧 修复：只使用 executedLevels 判断是否已执行，移除 partial_close_percentage 的判断
		// 原因：峰值回落和分批止盈是两个独立机制，都会累加 partial_close_percentage
		// 导致百分比超过100%后，所有阶段都被错误地跳过
		for (const stage of stages) {
			...
			// 🎯 关键修复：只检查 executedLevels，不再使用 partial_close_percentage 判断
			// 只要该阶段没有在 executedLevels 中，就继续检查是否达到触发条件
			if (executedLevels.has(stage.name)) {
				logger.debug(`${stage.name} 已执行（在executedLevels中），跳过`);
				continue;
			}
...
触发条件: 盈利从峰值${peakPnlPercent.toFixed(2)}%回退${drawdownPercent.toFixed(
		...
		if (isFullClose) {
			...
			const currentExecutedLevelsStr = String(
				posResult.rows[0]?.executed_levels || "[]",
			);
			...
			const currentDbQuantity = Number.parseFloat(
				(posInfoResult.rows[0]?.quantity as string) || "0",
			);
			const currentPartialClosePercentage = Number.parseFloat(
				(posInfoResult.rows[0]?.partial_close_percentage as string) || "0",
			);

			// 🔧 修复：基于实际平仓数量计算百分比，而不是简单累加配置的百分比
			// 获取初始持仓数量（从数据库的 initial_quantity 字段）
			const initialQuantityResult = await dbClient.execute({
				sql: "SELECT initial_quantity FROM positions WHERE symbol = ?",
				args: [symbol],
			});
			const dbInitialQuantity = Number.parseFloat(
				(initialQuantityResult.rows[0]?.initial_quantity as string) || "0",
			);

			// 使用数据库中的 initial_quantity 作为基准，如果没有则使用当前数据库数量
			const baseQuantity = dbInitialQuantity || currentDbQuantity;

			logger.info(
				`${symbol} 平仓百分比计算基准: initial_quantity=${dbInitialQuantity}, 使用基准=${baseQuantity}张`,
			);

			// 计算本次实际平仓的百分比（基于初始持仓数量）
			const thisClosePercent = (actualCloseQuantity / baseQuantity) * 100;

			// 累加本次平仓百分比
			const newPartialClosePercentage =
				currentPartialClosePercentage + thisClosePercent;

			logger.info(
				`${symbol} 更新已平仓百分比: ${currentPartialClosePercentage.toFixed(2)}% + ${thisClosePercent.toFixed(2)}% = ${newPartialClosePercentage.toFixed(2)}% (基准数量: ${baseQuantity}张, 本次平仓: ${actualCloseQuantity}张)`,
			);
			...
			await dbClient.execute({
				...
				args: [
					remainingQuantity,
					JSON.stringify(currentExecutedLevels),
					newPartialClosePercentage,
					symbol,
				],
			});
			...
			if (autoCloseEnabled || exitStrategy?.strategyType) {
				...
				if (exitResult.shouldClose) {
					...
					if (success) {
						...
						logger.info(
							`${symbol} 内存中记录已执行平仓级别: ${exitResult.level}`,
						);
