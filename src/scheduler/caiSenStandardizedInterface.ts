...
export class CaiSenStandardizedInterface extends EventEmitter {
  ...
  async getGapPeriodTakeoverStatus(
    ...
    try {
      ...
      const takeoverState = await this.gapPeriodTakeover.getTakeoverState(
        positionId
      );
  ...
  private validateStopProfitLossParameters(
    ...
    if (parameters.stopLoss) {
      ...
      // 止损百分比可以是负数（表示亏损百分比），只需要检查是否为数字
      if (
        parameters.stopLoss.type === "percentage" &&
        isNaN(parameters.stopLoss.value)
      ) {
        return {
          valid: false,
          errorMessage: "Stop loss percentage must be a valid number",
        };
      }
    }
