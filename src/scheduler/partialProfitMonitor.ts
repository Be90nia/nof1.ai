	...
	try {
		...
		if (order.id) {
			for (let retry = 0; retry < 5; retry++) {
				...
				try {
					...
					logger.warn(
						`查询分批止盈订单状态失败 (重试${retry + 1}/5): ${statusError.message}`,
					);
				}
			}
		}
		...
		if (actualExitPrice > 0) {
			try {
				...
				logger.info(
					`分批止盈平仓成交: 价格=${actualExitPrice.toFixed(2)}, 数量=${actualQuantity}, 盈亏=${pnl.toFixed(2)} USDT`,
				);
			} catch (calcError: any) {
...
分批止盈策略: 逐步锁定利润，保护已获收益`;
		...
		logger.info(
			`分批止盈平仓完成 ${symbol}，盈亏：${pnl >= 0 ? "+" : ""}${pnl.toFixed(2)} USDT`,
		);
...
async function checkPartialProfitConditions() {
	...
	try {
		...
		const dbResult = await dbClient.execute(
			"SELECT symbol, partial_close_percentage FROM positions",
		);
...
export function startPartialProfitMonitor() {
	...
	logger.info(
		`  自动平仓: ${autoCloseEnabled ? "✅ 启用（代码级保护）" : "❌ 禁用（由 AI 决策）"}`,
	);
