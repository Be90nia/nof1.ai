...
const caiSenMonitorState = {
  lastCheckTime: 0,
  checkCount: 0,
  crashDetectionCache: new Map<string, {
  ...
  pyramidAddCache: new Map<string, {
    lastAddPrice: number;
    lastAddTime: number;
    addCount: number;
  }>(),
 ...
 */
function detectCrash(klineData: any[], crashThreshold: number, timeWindow: number): {
  detected: boolean;
  highPrice: number;
  lowPrice: number;
  dropPercent: number;
 ...
 * @returns 七个分位价格水平
 */
function calculateSevenSegmentLevels(highPrice: number, lowPrice: number): number[] {
 ...
 * @returns 价格位置分析和交易信号
 */
function analyzePricePosition(currentPrice: number, sevenSegmentLevels: number[]): {
  position: string;
  zone: number;
  signal: string;
  confidence: string;
...
function checkPyramidAdd(
  ...
  if (priceChangePercent >= pyramidThreshold * 100) {
    // 检查是否已经加仓过
    if (!cache || (currentPrice - cache.lastAddPrice) / cache.lastAddPrice >= pyramidThreshold) {
      return {
        shouldAdd: true,
        addRatio: pyramidRatio,
        description: `价格有利移动${priceChangePercent.toFixed(2)}%，达到${(pyramidThreshold * 100).toFixed(1)}%阈值，建议加仓${(pyramidRatio * 100).toFixed(0)}%`,
      };
    }
  }
 ...
 * 记录蔡森策略监控数据到数据库
 */
async function recordCaiSenMonitorData(symbol: string, data: any): Promise<void> {
  try {
...
async function executePyramidAdd(
  ...
  try {
    ...
    caiSenMonitorState.pyramidAddCache.set(symbol, {
      ...
      addCount: (caiSenMonitorState.pyramidAddCache.get(symbol)?.addCount || 0) + 1,
    });
...
async function executeCaiSenMonitor(): Promise<void> {
  ...
  try {
    ...
    for (const position of positions) {
      ...
      // 1. 检查金字塔加仓
      const pyramidAddInfo = checkPyramidAdd(symbol, entryPrice, currentPrice, side);
      if (pyramidAddInfo) {
        logger.info(`${symbol} ${pyramidAddInfo.description}`);
        
        if (isCodeLevelProtectionEnabled()) {
          await executePyramidAdd(symbol, side, pyramidAddInfo.addRatio, pyramidAddInfo.description);
        } else {
      ...
      // 2. 暴跌检测和七分位分析
      const params = getCaiSenParams();
      const crashThreshold = params.sevenSegmentStrategy.crashDetectionThreshold;
      ...
      if (klineData.length > 0) {
        ...
        if (crashInfo && crashInfo.detected) {
          logger.info(`${symbol} 检测到暴跌: ${crashInfo.dropPercent.toFixed(2)}%`);
          ...
          // 计算七分位水平
          const sevenSegmentLevels = calculateSevenSegmentLevels(crashInfo.highPrice, crashInfo.lowPrice);
          
          // 分析当前价格位置
          const positionAnalysis = analyzePricePosition(currentPrice, sevenSegmentLevels);
          
          logger.info(`${symbol} 七分位分析: ${positionAnalysis.description}`);
          logger.info(`  七分位水平: [${sevenSegmentLevels.map(l => l.toFixed(6)).join(', ')}]`);
          ...
          if (positionAnalysis.signal !== "观望" && isCodeLevelProtectionEnabled()) {
            logger.warn(`${symbol} 七分位交易信号: ${positionAnalysis.signal}，信心度${positionAnalysis.confidence}`);
...
export function getCaiSenMonitorStatus(): {
  ...
  return {
    ...
    crashDetectionCache: Object.fromEntries(caiSenMonitorState.crashDetectionCache),
