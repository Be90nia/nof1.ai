...
import { CaiSenBatchClosingSystem, createCaiSenBatchClosingSystem, ClosingType, BatchConfig, AiClosingParams, DEFAULT_BATCH_CLOSING_CONFIG } from './caiSenBatchClosingSystem';
import { CaiSenAiParameterControl, createCaiSenAiParameterControl, ClosingParameterDetail } from './caiSenAiParameterControl';
import { IExchangeClient } from '../services/exchangeClient';
...
export class CaiSenBatchClosingInstructionRecognizer extends EventEmitter {
  ...
  constructor(
    ...
    logger.info('CaiSen Batch Closing Instruction Recognizer initialized', { config });
  }
  ...
  recognizeBatchClosingInstruction(
    ...
    try {
      ...
      const instructionId = `ins_batch_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
      ...
      if (this.config.enableInstructionDeduplication && this.isDuplicateInstruction(instruction)) {
        logger.warn('Duplicate instruction detected', { instructionId, positionId: instruction.positionId });
        return null;
      }
  ...
  recognizeEmergencyClosingInstruction(
    ...
    try {
      // 生成指令ID - Generate instruction ID
      const instructionId = `ins_emergency_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
  ...
  recognizeModifyBatchParamsInstruction(
    ...
    try {
      // 生成指令ID - Generate instruction ID
      const instructionId = `ins_modify_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
      
      // 查找现有指令 - Find existing instruction
      const existingInstructionIds = this.positionIdToInstructionIds.get(positionId) || [];
      ...
      if (!existingInstruction) {
        logger.error('No existing batch closing instruction found for modification', { positionId });
        return null;
      }
  ...
  recognizeCancelBatchClosingInstruction(
    ...
    try {
      // 生成指令ID - Generate instruction ID
      const instructionId = `ins_cancel_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
      ...
      if (!existingInstruction) {
        logger.error('No existing batch closing instruction found for cancellation', { positionId });
        return null;
      }
    ...
    try {
      ...
      switch (instruction.type) {
        ...
        case InstructionType.MODIFY_BATCH_PARAMS:
          isValid = await this.validateModifyBatchParamsInstruction(instruction);
          break;
          
        case InstructionType.CANCEL_BATCH_CLOSING:
          isValid = await this.validateCancelBatchClosingInstruction(instruction);
          break;
      ...
      if (instruction) {
        instruction.status = InstructionStatus.FAILED;
        instruction.updatedAt = Date.now();
        instruction.errorMessage = error instanceof Error ? error.message : String(error);
      }
  ...
  async executeInstruction(instructionId: string): Promise<boolean> {
    try {
      ...
      switch (instruction.type) {
        ...
        case InstructionType.EMERGENCY_CLOSING:
          isSuccess = await this.executeEmergencyClosingInstruction(instruction);
          break;
          
        case InstructionType.MODIFY_BATCH_PARAMS:
          isSuccess = await this.executeModifyBatchParamsInstruction(instruction);
          break;
          
        case InstructionType.CANCEL_BATCH_CLOSING:
          isSuccess = await this.executeCancelBatchClosingInstruction(instruction);
          break;
   ...
   */
  getInstructionsByPositionId(positionId: string, status?: InstructionStatus): BatchClosingInstruction[] {
    const instructionIds = this.positionIdToInstructionIds.get(positionId) || [];
    const instructions: BatchClosingInstruction[] = [];
    
    for (const instructionId of instructionIds) {
      const instruction = this.instructions.get(instructionId);
      if (instruction && (status === undefined || instruction.status === status)) {
   ...
   * @returns BatchClosingInstruction[] 指令列表 - List of instructions
   */
  getInstructionsByStatus(status: InstructionStatus): BatchClosingInstruction[] {
  ...
  getAllInstructions(status?: InstructionStatus): BatchClosingInstruction[] {
    ...
    if (status !== undefined) {
      return instructions.filter(instruction => instruction.status === status);
    }
  ...
  private parseInstructionData(instructionData: any): any | null {
    try {
      ...
      if (!instructionData.positionId || !instructionData.symbol || !instructionData.direction || !instructionData.totalQuantity) {
        logger.error('Missing required fields in instruction data', { instructionData });
        return null;
      }
      ...
      if (instructionData.batchConfig) {
        ...
        batchConfig = {
          ...
          triggerCondition: {
            triggerType: 'price',
            triggerValue: instructionData.direction === 'long' 
      ...
      if (instructionData.triggerConditions && Array.isArray(instructionData.triggerConditions)) {
        triggerConditions = instructionData.triggerConditions.map((condition: any, index: number) => ({
   ...
   * @private
   */
  private isDuplicateInstruction(instruction: BatchClosingInstruction): boolean {
    try {
      const positionInstructions = this.getInstructionsByPositionId(instruction.positionId);
      
      for (const existingInstruction of positionInstructions) {
        // 检查时间窗口内的相同类型指令
        // Check for same type instructions within time window
        if (existingInstruction.type === instruction.type &&
            ...
            Math.abs(existingInstruction.createdAt - instruction.createdAt) < this.config.deduplicationTimeWindow) {
          return true;
        }
      }
  ...
  private updateIndexes(instruction: BatchClosingInstruction): void {
    try {
      ...
      const positionInstructionIds = this.positionIdToInstructionIds.get(instruction.positionId)!;
      ...
      for (const [status, instructionIds] of this.statusToInstructionIds.entries()) {
   ...
   * @private
   */
  private async validateBatchClosingInstruction(instruction: BatchClosingInstruction): Promise<boolean> {
    try {
      // 验证基本参数 - Validate basic parameters
      if (instruction.totalQuantity <= 0) {
        logger.error('Invalid total quantity', { instructionId: instruction.instructionId, totalQuantity: instruction.totalQuantity });
        return false;
      }
      
      // 验证分批配置 - Validate batch configuration
      if (!instruction.batchConfig) {
        logger.error('无效的批量配置', { instructionId: instruction.instructionId });
        return false;
      }
      
      // 验证平仓比例 - Validate closing ratio
      if (instruction.batchConfig.closingRatio <= 0 || instruction.batchConfig.closingRatio > 1) {
      ...
      if (instruction.batchConfig.closingQuantity <= 0 || instruction.batchConfig.closingQuantity > instruction.totalQuantity) {
      ...
      logger.error('验证批量平仓指令时出错', { error, instructionId: instruction.instructionId });
      return false;
    }
  }
   ...
   * @private
   */
  private async validateEmergencyClosingInstruction(instruction: BatchClosingInstruction): Promise<boolean> {
    try {
      // 验证基本参数 - Validate basic parameters
      if (instruction.totalQuantity <= 0) {
        logger.error('Invalid total quantity', { instructionId: instruction.instructionId, totalQuantity: instruction.totalQuantity });
      ...
      logger.error('验证紧急平仓指令时出错', { error, instructionId: instruction.instructionId });
      return false;
    }
  }
   ...
   * @private
   */
  private async validateModifyBatchParamsInstruction(instruction: BatchClosingInstruction): Promise<boolean> {
    try {
      // 验证基本参数 - Validate basic parameters
      if (instruction.totalQuantity <= 0) {
        logger.error('Invalid total quantity', { instructionId: instruction.instructionId, totalQuantity: instruction.totalQuantity });
      ...
      logger.error('验证修改分批参数指令时出错', { error, instructionId: instruction.instructionId });
      return false;
    }
  }
   ...
   * @private
   */
  private async validateCancelBatchClosingInstruction(instruction: BatchClosingInstruction): Promise<boolean> {
    try {
      // 验证基本参数 - Validate basic parameters
      if (instruction.totalQuantity <= 0) {
        logger.error('Invalid total quantity', { instructionId: instruction.instructionId, totalQuantity: instruction.totalQuantity });
      ...
      logger.error('验证取消分批平仓指令时出错', { error, instructionId: instruction.instructionId });
      return false;
    }
  }
   ...
   * @private
   */
  private async executeBatchClosingInstruction(instruction: BatchClosingInstruction): Promise<boolean> {
    try {
      ...
      const success = await this.batchClosingSystem.setAiClosingParams(aiParams);
      
      if (success) {
        // 触发批次执行 - Trigger batch execution
        await this.batchClosingSystem.triggerBatchExecution(instruction.batchConfig.batchId);
      }
      
      return success;
    } catch (error) {
      logger.error('执行批量平仓指令时出错', { error, instructionId: instruction.instructionId });
      return false;
    }
  }
   ...
   * @private
   */
  private async executeEmergencyClosingInstruction(instruction: BatchClosingInstruction): Promise<boolean> {
    try {
      // 构建AI平仓参数 - Build AI closing parameters
      const aiParams: AiClosingParams = {
        paramId: instruction.instructionId,
      ...
      logger.error('执行紧急平仓指令时出错', { error, instructionId: instruction.instructionId });
      return false;
    }
  }
   ...
   * @private
   */
  private async executeModifyBatchParamsInstruction(instruction: BatchClosingInstruction): Promise<boolean> {
    try {
      // 取消现有的AI平仓参数 - Cancel existing AI closing parameters
      await this.batchClosingSystem.cancelAiClosingParams(instruction.instructionId);
      ...
      logger.error('执行修改分批参数指令时出错', { error, instructionId: instruction.instructionId });
      return false;
    }
  }
   ...
   * @private
   */
  private async executeCancelBatchClosingInstruction(instruction: BatchClosingInstruction): Promise<boolean> {
    try {
      // 取消AI平仓参数 - Cancel AI closing parameters
      const success = await this.batchClosingSystem.cancelAiClosingParams(instruction.instructionId);
      
      return success;
    } catch (error) {
      logger.error('执行取消分批平仓指令时出错', { error, instructionId: instruction.instructionId });
      return false;
    }
  }
  ...
  private async processPendingValidations(): Promise<void> {
    try {
      const pendingInstructions = this.getInstructionsByStatus(InstructionStatus.PENDING);
  ...
  private async processPendingExecutions(): Promise<void> {
    try {
      const validatedInstructions = this.getInstructionsByStatus(InstructionStatus.VALIDATED);
  ...
  private async checkInstructionExpiration(): Promise<void> {
    try {
      ...
      for (const instruction of activeInstructions) {
        ...
        this.emit('instructionExpired', { instructionId: instruction.instructionId, instruction });
  ...
  /**
   * 销毁指令识别系统
   * Destroy instruction recognition system
   */
  destroy(): void {
    // 停止定时器 - Stop timers
    if (this.validationTimer) {
...
export const createCaiSenBatchClosingInstructionRecognizer = (
  ...
  const batchClosingSystem = createCaiSenBatchClosingSystem(DEFAULT_BATCH_CLOSING_CONFIG, {} as StrategyParams);
  const aiParameterControl = createCaiSenAiParameterControl(exchangeClient, {} as StrategyParams, batchClosingSystem);
