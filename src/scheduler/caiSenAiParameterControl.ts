...
import { StrategyParams } from '../strategies/types.js';
import { CaiSenBatchClosingSystem, AiClosingParams, BatchConfig } from './caiSenBatchClosingSystem';
...
export class CaiSenAiParameterControl extends EventEmitter {
  ...
  async setClosingParameters(
    ...
    try {
      ...
      if (this.config.enableParameterValidation && !this.validateClosingParameters(closingParams)) {
      ...
      const paramDetail: ClosingParameterDetail = {
        ...
        tags: options.tags,
        status: effectiveTime <= now ? ParameterStatus.ACTIVE : ParameterStatus.DRAFT,
        closingParams
      };
  ...
  async activateClosingParameter(paramId: string): Promise<boolean> {
    try {
      ...
      await this.batchClosingSystem.setAiClosingParams(paramDetail.closingParams);
  ...
  async cancelClosingParameter(paramId: string): Promise<boolean> {
    try {
      ...
      if (this.activeClosingParamId === paramId) {
        ...
        await this.batchClosingSystem.cancelAiClosingParams(paramDetail.closingParams.paramId);
      }
  ...
  async setRiskParameters(
    ...
    try {
      // 验证参数 - Validate parameters
      if (this.config.enableParameterValidation && !this.validateRiskParameters(riskThresholds)) {
  ...
  async activateRiskParameter(paramId: string): Promise<boolean> {
    try {
      ...
      const now = Date.now();
      if (paramDetail.effectiveTime > now || paramDetail.expiresAt <= now) {
        logger.error('Cannot activate parameters outside effective time range', { 
          paramId,
          effectiveTime: paramDetail.effectiveTime,
          expiresAt: paramDetail.expiresAt,
          now
   ...
   */
  async updateConfig(newConfig: Partial<ParameterControlConfig>): Promise<boolean> {
    try {
  ...
  private async checkExpiredParameters(): Promise<void> {
    try {
      ...
      for (const [paramId, paramDetail] of this.closingParams.entries()) {
        if (paramDetail.status === ParameterStatus.ACTIVE && paramDetail.expiresAt <= now) {
  ...
  private async cleanupExpiredParameters(): Promise<void> {
    try {
      ...
      const expiredClosingParams = Array.from(this.closingParams.entries())
        .filter(([_, paramDetail]) => paramDetail.status === ParameterStatus.EXPIRED)
        .sort(([_, a], [__, b]) => a.expiresAt - b.expiresAt); // 按过期时间排序
      
      if (expiredClosingParams.length > this.config.maxParameterHistory) {
        const toDelete = expiredClosingParams.slice(0, expiredClosingParams.length - this.config.maxParameterHistory);
      ...
      if (expiredRiskParams.length > this.config.maxParameterHistory) {
        const toDelete = expiredRiskParams.slice(0, expiredRiskParams.length - this.config.maxParameterHistory);
        for (const [paramId] of toDelete) {
   ...
   * @private
   */
  private validateRiskParameters(riskThresholds: RiskParameterDetail['riskThresholds']): boolean {
    // 检查基本字段 - Check basic fields
    if (riskThresholds.maxTotalLossPercent <= 0 || 
 ...
 * 
 * @param {IExchangeClient} exchangeClient - 交易所客户端 Exchange client
 * @param {CaiSenStrategyConfig} strategyConfig - 策略配置 Strategy configuration
 * @param {CaiSenBatchClosingSystem} batchClosingSystem - 批量平仓系统 Batch closing system
 * @param {Partial<ParameterControlConfig>} config - 参数控制机制配置 Parameter control mechanism configuration
 * @returns {CaiSenAiParameterControl} 蔡森策略AI参数控制机制实例 CaiSen Strategy AI Parameter Control Mechanism instance
 * 
 * 示例 Example:
 * ```typescript
 * import { createCaiSenAiParameterControl } from './caiSenAiParameterControl';
 * import { IExchangeClient, createExchangeClient } from '../services/exchangeClient';
 * import { CaiSenStrategyConfig } from '../strategies/caiSen';
 * import { CaiSenBatchClosingSystem } from './caiSenBatchClosingSystem';
 * 
 * const exchangeClient = createExchangeClient();
 * const strategyConfig: CaiSenStrategyConfig = { \/* strategy config *\/ };
 * const batchClosingSystem = new CaiSenBatchClosingSystem(exchangeClient);
 * 
