...
// 基础资金费率接口
export interface BaseFundingRate {
  symbol?: string;
  contract?: string;
  fundingRate?: string;
...
// 基础持仓历史接口
export interface BasePositionHistory {
  symbol?: string;
  contract?: string;
  side?: string;
  size?: string;
...
export interface OkxFundingRate {
  ...
  nextFundingRate?: string; // 下次资金费率
  fundingTime?: string; // 资金费率计费时间
  nextFundingTime?: string; // 下次资金费率计费时间
  [key: string]: any;
}
...
export interface OkxPositionHistory {
  ...
  posSide?: string; // 持仓方向
  pos?: string; // 持仓数量
  baseBal?: string; // 交易货币余额
  quoteBal?: string; // 计价货币余额
...
export interface ExchangeConverter {
  ...
  // 持仓历史转换
  convertPositionHistory(
    data: any,
    sourceExchange: ExchangeType
  ): BasePositionHistory;
}
