...
/**
 * OKX WebSocket 客户端
 * 支持订阅 ticker、candles、books、trades 等行情数据
 */
export class OkxWebSocketClient {
  ...
  private orderBookCache: Map<string, OrderBookCache> = new Map();
  ...
  private handleMessage(message: string): void {
    try {
      ...
      if (data.data && Array.isArray(data.data)) {
        ...
        } else if (channel === "trades" && instId) {
          this.handleTradeUpdate(instId, data.data);
        }
   ...
   * 处理订单簿更新
   */
  private handleOrderBookUpdate(instId: string, book: any): void {
    const existing = this.orderBookCache.get(instId);
    const bids = book.bids || [];
    const asks = book.asks || [];

    if (existing) {
      // 保存之前的订单簿数据，用于计算变化率
      existing.prevBids = [...existing.bids];
      existing.prevAsks = [...existing.asks];
      ...
      this.orderBookCache.set(instId, {
        bids,
        asks,
        lastUpdate: Date.now(),
        prevBids: [],
   ...
   * 处理交易更新
   */
  private handleTradeUpdate(instId: string, trades: any[]): void {
    const existing = this.tradeCache.get(instId);

    if (existing) {
      // 合并新的交易数据
      const newTrades = trades.filter((trade) => {
        const tradeId = trade.tradeId;
        return !existing.data.some((t) => t.tradeId === tradeId);
      });

      // 添加新交易数据
      existing.data = [...newTrades, ...existing.data];

      // 保持最多 500 条数据
      if (existing.data.length > 500) {
        existing.data = existing.data.slice(0, 500);
      }
      existing.lastUpdate = Date.now();
    } else {
      this.tradeCache.set(instId, {
        data: trades,
        lastUpdate: Date.now(),
      });
    }
   ...
   * 获取缓存的订单簿数据
   */
  getCachedOrderBook(instId: string): any | null {
    const cache = this.orderBookCache.get(instId);

    // 如果数据超过 5 秒，认为过期
    if (cache && Date.now() - cache.lastUpdate < 5000) {
      return {
        bids: cache.bids,
        asks: cache.asks,
        prevBids: cache.prevBids,
        prevAsks: cache.prevAsks,
   ...
   * 获取缓存的交易数据
   */
  getCachedTrades(instId: string): any[] | null {
    const cache = this.tradeCache.get(instId);

    // 如果数据超过 2 分钟，认为过期
    if (cache && Date.now() - cache.lastUpdate < 120000) {
      return cache.data;
    }
   ...
   * 等待 ticker 数据
   */
  async waitForTicker(instId: string, timeout = 5000): Promise<any> {
    // 先检查缓存
