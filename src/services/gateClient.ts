...
import { type Position } from "./exchangeClient";
...
export class GateClient {
   ...
   */
  async getPositions(retries = 2): Promise<Position[]> {
    let lastError: any;
    ...
    for (let i = 0; i <= retries; i++) {
      try {
        ...
        return filteredPositions.map((p: any) => {
          ...
          const markPrice = Number.parseFloat(p.markPrice || "0");
          const leverage = Number.parseFloat(p.leverage || "1");
          ...
          return {
            ...
            liqPrice: Number.parseFloat(p.liqPrice || "0"),
            unrealisedPnl: Number.parseFloat(p.unrealisedPnl || "0").toString(),
            positionId: p.contract,
            positionAmt: size,
            pnlPercent,
            id: p.id || p.contract,
            liq_price: Number.parseFloat(p.liqPrice || "0"),
            symbol: p.contract?.split("_")[0],
  ...
  async closePosition(params: {
    ...
    try {
      ...
      // 确定平仓方向（与持仓方向相反）
      // 使用side字段判断持仓方向，比使用size的正负号更可靠
      const isLong = targetPosition.side === "long" || positionSizeBig.gt(0);
