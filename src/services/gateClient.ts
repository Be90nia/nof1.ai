...
export class GateClient {
  ...
  private calculateATR(candles: any[], period: number): number {
    ...
    for (let i = 1; i < candles.length; i++) {
      ...
      const high = parseFloat(current.h);
      const low = parseFloat(current.l);
      const prevClose = parseFloat(previous.c);
  ...
  async getFearAndGreedIndex(baseSymbol = "BTC") {
    try {
      ...
      const openPrice24h = parseFloat(candles24h[0].o);
      ...
      const fundingRate = parseFloat(fundingRateData.r) * 100;
