...
import {
  ...
  InterfaceCallResult,
} from "../scheduler/caiSenStandardizedInterface";
...
export function createCaiSenTradingTools(
  ...
  async function getBatchClosingStatus(
    ...
    try {
      const result = await interfaceInstance.getAllStatusByPositionId(
        positionId
      );
  ...
  async function getStopProfitLossStatus(
    ...
    try {
      const result = await interfaceInstance.getThresholdsByPositionId(
        positionId
      );
