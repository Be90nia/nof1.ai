...
async function _setDynamicStopLossParams(
	...
	try {
		...
		// 验证和转换 conditions 参数
		const processedConditions: DynamicStopLossCondition[] = [];
		if (conditions) {
			// 确保 conditions 是数组
...
async function _setPositionExitStrategy(
	...
	try {
		...
		// 🔧 修复：同时更新 positions 表的 exit_strategy 字段
		// 监控系统从 positions.exit_strategy 读取参数，必须同步更新
		try {
			const positionCheck = await dbClient.execute(
				"SELECT symbol FROM positions WHERE symbol = ?",
				[symbol],
			);

			if (positionCheck.rows.length > 0) {
				// 构建完整的 exitStrategy 对象（不包含 lastUpdated）
				const exitStrategyForPosition = {
					strategyType,
					enabled,
					...(partialTakeProfit && { partialTakeProfit }),
					...(dynamicStopLoss && { dynamicStopLoss }),
				...
				// 持仓存在，更新 exit_strategy 和清空 executed_levels
				await dbClient.execute(
					"UPDATE positions SET exit_strategy = ?, executed_levels = '[]' WHERE symbol = ?",
					[JSON.stringify(exitStrategyForPosition), symbol],
				);

				logger.info({
					action: "update_position_exit_strategy",
					function: "setPositionExitStrategy",
					symbol,
					message:
						"已更新持仓的 exit_strategy 字段并清空已执行级别，监控系统将使用新参数",
				});
			}
		} catch (error) {
			// 如果更新失败，记录警告但不影响主流程
			logger.warn({
				action: "update_position_exit_strategy_error",
				function: "setPositionExitStrategy",
				symbol,
				error: (error as Error).message,
				message: "更新持仓 exit_strategy 失败，但不影响参数设置",
			});
		}
