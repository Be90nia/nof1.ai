...
export const openPositionTool = createTool({
	name: "openPosition",
	description:
		"开仓 - 做多或做空指定币种（使用市价单，立即以当前市场价格成交）。IMPORTANT: 开仓前必须先用getAccountBalance和getPositions工具查询可用资金和现有持仓，避免资金不足。交易手续费约0.05%，避免频繁交易。开仓时不设置止盈止损，你需要在每个周期主动决策是否平仓。",
	parameters: z.object({
		...
		leverage: z
			.number()
			.min(1)
			.max(RISK_PARAMS.MAX_LEVERAGE)
			.describe(
				`杠杆倍数（1-${RISK_PARAMS.MAX_LEVERAGE}倍，根据环境变量MAX_LEVERAGE配置）`,
			),
		...
		try {
			...
			if (existingPosition) {
				...
				if (existingSide !== side) {
					return {
						success: false,
						message: `${symbol} 已有${
							existingSide === "long" ? "多" : "空"
						}单持仓，禁止同时持有双向持仓。请先平掉${
							existingSide === "long" ? "多" : "空"
						}单后再开${side === "long" ? "多" : "空"}单。`,
					};
				}

				// 如果方向相同，允许加仓（但需要注意总持仓限制）
				logger.info(
					`${symbol} 已有${side === "long" ? "多" : "空"}单持仓，允许加仓`,
				);
			}
			...
			if (recentCloseResult.rows.length > 0) {
				const closeTime = new Date(
					recentCloseResult.rows[0].timestamp as string,
				).getTime();
				const now = Date.now();
				...
				if (minutesSinceClose < cooldownMinutes) {
					return {
						success: false,
						message: `拒绝开仓 ${symbol}：该币种在 ${minutesSinceClose.toFixed(
							1,
						)} 分钟前刚平仓，需要等待至少 ${cooldownMinutes.toFixed(
							1,
						)} 分钟（半个交易周期）后才能重新开仓。这是为了防止频繁反复交易，造成不必要的手续费损失。`,
					};
				}

				logger.info(
					`${symbol} 距离上次平仓已 ${minutesSinceClose.toFixed(
						1,
					)} 分钟，通过冷静期检查（冷静期：${cooldownMinutes.toFixed(1)}分钟）`,
				);
			}
			...
			if (totalExposure > maxAllowedExposure) {
				return {
					success: false,
					message: `新开仓将导致总敞口 ${totalExposure.toFixed(
						2,
					)} USDT 超过限制 ${maxAllowedExposure.toFixed(2)} USDT（账户净值的${
			...
			const maxSinglePosition = totalBalance * 0.3; // 30%
			if (amountUsdt > maxSinglePosition) {
				logger.warn(
					`开仓金额 ${amountUsdt.toFixed(
						2,
					)} USDT 超过建议仓位 ${maxSinglePosition.toFixed(
						2,
					)} USDT（账户净值的30%）`,
				);
			}
			...
			if (hourUTC >= 2 && hourUTC <= 6) {
				logger.warn(
					`⚠️  当前处于低流动性时段 (UTC ${hourUTC}:00)，建议谨慎交易`,
				);
				// 在低流动性时段降低仓位
			...
			try {
				...
				if (orderBook && orderBook.bids && orderBook.bids.length > 0) {
					...
					if (bidDepth < requiredDepth) {
						return {
							success: false,
							message: `流动性不足：订单簿深度 ${bidDepth.toFixed(
								2,
							)} USDT < 所需 ${requiredDepth.toFixed(2)} USDT`,
						};
					}

					logger.info(
						`✅ 流动性检查通过：订单簿深度 ${bidDepth.toFixed(
							2,
						)} USDT >= 所需 ${requiredDepth.toFixed(2)} USDT`,
					);
				}
			...
			if (volatilityLevel === "high") {
				...
				adjustedLeverage = Math.max(
					1,
					Math.round(leverage * adjustment.leverageFactor),
				);
				adjustedAmountUsdt = Math.max(
					10,
					amountUsdt * adjustment.positionFactor,
				);
				logger.info(
					`🌊 高波动市场 (ATR ${atrPercent.toFixed(
						2,
					)}%)：杠杆 ${leverage}x → ${adjustedLeverage}x，仓位 ${amountUsdt.toFixed(
						0,
					)} → ${adjustedAmountUsdt.toFixed(0)} USDT`,
				);
				...
				logger.info(
					`🌊 正常波动市场 (ATR ${atrPercent.toFixed(2)}%)：保持原始参数`,
				);
			}
			...
			const maxSize = Number.parseFloat(
				contractInfo.orderSizeMax || contractInfo.order_size_max || "1000000",
			);
			...
			let quantity =
				(adjustedAmountUsdt * leverage) / (quantoMultiplier * currentPrice);
			...
			if (Math.abs(size) < minSize) {
				...
				return {
					success: false,
					message: `计算的数量 ${Math.abs(
						size,
					)} 张小于最小限制 ${minSize} 张，需要至少 ${minMargin.toFixed(
						2,
					)} USDT 保证金（当前${adjustedAmountUsdt.toFixed(
						2,
					)} USDT，杠杆${leverage}x）`,
				};
			}
			...
			// 计算实际使用的保证金
			const actualMargin =
				(Math.abs(size) * quantoMultiplier * currentPrice) / leverage;

			logger.info(
				`开仓 ${symbol} ${side === "long" ? "做多" : "做空"} ${Math.abs(
					size,
				)}张 (杠杆${leverage}x)`,
			);
				...
				while (retryCount < maxRetries) {
					try {
						...
						if (
							...
							orderDetail.price &&
							Number.parseFloat(orderDetail.price) > 0
						) {
							actualFillPrice = Number.parseFloat(orderDetail.price);
						}

						logger.info(
							`成交: ${actualFillSize}张 @ ${actualFillPrice.toFixed(2)} USDT`,
						);

						//  验证成交价格的合理性（滑点保护）
						const priceDeviation =
							Math.abs(actualFillPrice - currentPrice) / currentPrice;
						if (priceDeviation > 0.02) {
							// 滑点超过2%，拒绝此次交易（回滚）
							logger.error(
								`❌ 成交价偏离超过2%: ${currentPrice.toFixed(
									2,
								)} → ${actualFillPrice.toFixed(2)} (偏离 ${(
									priceDeviation * 100
								).toFixed(2)}%)，拒绝交易`,
							);
							...
							return {
								success: false,
								message: `开仓失败：成交价偏离超过2% (${currentPrice.toFixed(
									2,
								)} → ${actualFillPrice.toFixed(2)})，已拒绝交易`,
							};
						}
						...
						if (finalOrderStatus === "cancelled" || actualFillSize === 0) {
							return {
								success: false,
								message: `开仓失败：订单${
									finalOrderStatus === "cancelled" ? "被取消" : "未成交"
								}（订单ID: ${order.id}）`,
							};
						}
						...
						if (retryCount >= maxRetries) {
							logger.error(
								`获取订单详情失败（重试${retryCount}次）: ${error.message}`,
							);
							// 如果无法获取订单详情，使用预估值继续
							logger.warn(
								`使用预估值继续: 数量=${Math.abs(size)}, 价格=${currentPrice}`,
							);
							actualFillSize = Math.abs(size);
							actualFillPrice = currentPrice;
						} else {
							logger.warn(
								`获取订单详情失败，${retryCount}/${maxRetries} 次重试...`,
							);
			...
			//  使用实际成交数量和价格记录到数据库
			const finalQuantity =
				actualFillSize > 0 ? actualFillSize : Math.abs(size);
			...
			while (retryCount < maxRetries) {
				try {
					...
					const gatePosition = positions.find(
						(p: any) =>
					...
					if (retryCount >= maxRetries) {
						logger.error(
							`❌ 警告：Gate.io 查询显示持仓为0，但订单状态为 ${finalOrderStatus}`,
						);
						logger.error(
					...
					logger.warn(
						`获取持仓失败（重试${retryCount + 1}/${maxRetries}）: ${error}`,
					);
					retryCount++;
				}
			}
			...
			if (existingResult.rows.length > 0) {
                ...
                (symbol, quantity, entry_price, current_price, liquidation_price, unrealized_pnl, 
                 leverage, side, profit_target, stop_loss, tp_order_id, sl_order_id, entry_order_id, opened_at, executed_levels, initial_quantity)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
					args: [
						symbol,
						finalQuantity,
			...
			return {
				...
				message: `✅ 成功开仓 ${symbol} ${
					side === "long" ? "做多" : "做空"
				} ${Math.abs(size)} 张 (${contractAmount.toFixed(
					4,
				)} ${symbol})，成交价 ${actualFillPrice.toFixed(
					2,
				)}，保证金 ${actualMargin.toFixed(
					2,
				)} USDT，杠杆 ${leverage}x。⚠️ 未设置止盈止损，请在每个周期主动决策是否平仓。`,
			};
		} catch (error: any) {
			return {
				success: false,
		...
		try {
			...
			if (dbPositionResult.rows.length > 0) {
				...
				if (holdingMinutes < minHoldingMinutes) {
					return {
						success: false,
						message: `拒绝平仓 ${symbol}：持仓时间仅 ${holdingMinutes.toFixed(
							1,
						)} 分钟，少于最小持仓时间 ${minHoldingMinutes.toFixed(
							1,
						)} 分钟。请等待至少半个交易周期后再评估平仓。这是为了防止在同一周期内刚开仓就立即平仓，造成不必要的手续费损失。`,
					};
				}

				logger.info(
					`${symbol} 持仓时间: ${holdingMinutes.toFixed(
						1,
					)} 分钟，通过最小持仓时间检查`,
				);
			}
			...
			const exchangeEntryPrice = Number.parseFloat(
				gatePosition?.entryPrice || "0",
			);
			...
			let entryPrice =
				dbPosition?.average_entry_price ||
				dbPosition?.entry_price ||
				exchangeEntryPrice;
			...
			logger.info(
				`预估盈亏: ${grossPnl >= 0 ? "+" : ""}${grossPnl.toFixed(
					2,
				)} USDT (价格变动: ${priceChange.toFixed(4)})`,
			);
			...
			logger.info(
				`平仓 ${symbol} ${
					side === "long" ? "做多" : "做空"
				} ${closeSize}张 (入场: ${entryPrice.toFixed(
					2,
				)}, 当前: ${currentPrice.toFixed(2)})`,
			);

			// 使用优化后的closePosition方法执行平仓，确保精确计算和平仓结果验证
			const order = await client.closePosition({
				contract,
				size: percentage === 100 ? undefined : closeSize, // 100%平仓则不指定size，否则指定平仓数量
			});

			// 设置默认值
			const actualExitPrice = currentPrice;
			const actualCloseSize = closeSize;
			const finalOrderStatus = order?.status || "filled";

			// 计算实际盈亏 - 使用已有的合约乘数，重新计算价格变化和盈亏
			...
			// 盈亏 = 价格变化 * 张数 * 合约乘数
			const actualGrossPnl =
				actualPriceChange * actualCloseSize * quantoMultiplier;

			//  扣除手续费（开仓 + 平仓）
			// 开仓手续费 = 开仓名义价值 * 0.05%
			const actualOpenFee =
				entryPrice * actualCloseSize * quantoMultiplier * 0.0005;
			// 平仓手续费 = 平仓名义价值 * 0.05%
			const actualCloseFee =
				actualExitPrice * actualCloseSize * quantoMultiplier * 0.0005;
			// 总手续费
			const actualTotalFees = actualOpenFee + actualCloseFee;

			// 净盈亏 = 毛盈亏 - 总手续费
			pnl = actualGrossPnl - actualTotalFees;

			logger.info(
				`成交: ${actualCloseSize}张 @ ${actualExitPrice.toFixed(2)} USDT`,
			);
			...
			// 开仓手续费 = 开仓名义价值 * 0.05%
			const dbOpenFee =
				entryPrice * actualCloseSize * dbQuantoMultiplier * 0.0005;
			// 平仓手续费 = 平仓名义价值 * 0.05%
			const dbCloseFee =
				actualExitPrice * actualCloseSize * dbQuantoMultiplier * 0.0005;
			// 总手续费
			const totalFee = dbOpenFee + dbCloseFee;

			// 🔥 关键验证：检查盈亏计算是否正确
			const notionalValue =
				actualExitPrice * actualCloseSize * dbQuantoMultiplier;
			const priceChangeCheck =
				side === "long"
					? actualExitPrice - entryPrice
					: entryPrice - actualExitPrice;
			const expectedPnl =
				priceChangeCheck * actualCloseSize * dbQuantoMultiplier - totalFee;
			...
			if (Math.abs(pnl - notionalValue) < Math.abs(pnl - expectedPnl)) {
				...
				logger.error(
					`  开仓价: ${entryPrice}, 平仓价: ${actualExitPrice}, 数量: ${actualCloseSize}, 合约乘数: ${dbQuantoMultiplier}`,
				);
				logger.error(
					`  价格变动: ${priceChangeCheck.toFixed(
						4,
			...
			logger.info(
				`  开仓价: ${entryPrice.toFixed(4)}, 平仓价: ${actualExitPrice.toFixed(
					4,
				)}, 数量: ${actualCloseSize}张`,
			);
			logger.info(
				`  价格变动: ${priceChangeCheck.toFixed(
					4,
				)}, 合约乘数: ${dbQuantoMultiplier}`,
			);
			logger.info(
				`  毛盈亏: ${(
					priceChangeCheck * actualCloseSize * dbQuantoMultiplier
				).toFixed(2)} USDT`,
			);
			logger.info(
				`  开仓手续费: ${dbOpenFee.toFixed(
					4,
				)} USDT, 平仓手续费: ${dbCloseFee.toFixed(4)} USDT`,
			);
			...
			if (posResult.rows.length > 0) {
				...
				if (dbPosition.sl_order_id) {
					try {
						...
						// 订单可能已经不存在或已被取消
						logger.warn(
							`无法取消止损订单 ${dbPosition.sl_order_id}: ${e.message}`,
						);
					}
				}
				...
				if (dbPosition.tp_order_id) {
					try {
						...
						// 订单可能已经不存在或已被取消
						logger.warn(
							`无法取消止盈订单 ${dbPosition.tp_order_id}: ${e.message}`,
						);
					}
				}
			}
			...
			return {
				...
				fee: totalFee, // 总手续费
				totalBalance,
				message: `成功平仓 ${symbol} ${actualCloseSize} 张，入场价 ${entryPrice.toFixed(
					4,
				)}，平仓价 ${actualExitPrice.toFixed(4)}，净盈亏 ${
					pnl >= 0 ? "+" : ""
				}${pnl.toFixed(2)} USDT (已扣手续费 ${totalFee.toFixed(
					2,
				)} USDT)，当前总资产 ${totalBalance.toFixed(2)} USDT`,
			};
		} catch (error: any) {
		...
		try {
			...
			await dbClient.execute({
				...
				args: [
					...
					takeProfit,
					closingType,
					batchParams ? JSON.stringify(batchParams) : null,
					symbol,
				],
			});
	...
	execute: async ({ symbol, percentages, triggerConditions }) => {
		try {
			...
			return {
				...
				percentages,
				triggerConditions,
				message: `成功为 ${symbol} 设置分批平仓策略`,
			};
		} catch (error: any) {
			return {
				success: false,
				error: error.message,
				message: `设置分批平仓策略失败: ${error.message}`,
			};
		}
	},
});
