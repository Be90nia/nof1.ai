  ...
  execute: async ({ symbol, side, leverage, amountUsdt }) => {
    ...
    try {
        ...
        if (minutesSinceClose < intervalMinutes) {
          return {
            ...
            message: `拒绝开仓 ${symbol}：该币种在 ${minutesSinceClose.toFixed(
              1
            )} 分钟前刚平仓，需要等待至少 ${intervalMinutes} 分钟（一个完整交易周期）后才能重新开仓。这是为了防止同一周期内频繁交易，造成不必要的手续费损失和情绪化交易。`,
          };
        }
      ...
      for (const pos of activePositions) {
        ...
        // 获取合约乘数
        const posQuantoMultiplier = await getQuantoMultiplier(
          pos.contract || ""
        );
        ...
        while (retryCount < maxRetries) {
          try {
            ...
            if (
              ...
              orderDetail.price &&
              Number.parseFloat(orderDetail.price.toString()) > 0
            ) {
      ...
      if (order.id) {
        ...
        while (retryCount < maxRetries) {
          try {
            ...
            //  验证成交价格的合理性（滑点保护）
            const priceDeviation =
              Math.abs(actualExitPrice - currentPrice) / currentPrice;
            if (priceDeviation > 0.03) {
              // 平仓时允许3%滑点（比开仓宽松，因为可能是紧急止损）
              logger.warn(
                `⚠️ 平仓成交价偏离超过3%: ${currentPrice.toFixed(
                  2
                )} → ${actualExitPrice.toFixed(2)} (偏离 ${(
                  priceDeviation * 100
                ).toFixed(2)}%)`
              );
            }
            ...
            if (retryCount >= maxRetries) {
              logger.error(
                `获取平仓订单详情失败（重试${retryCount}次）: ${error.message}`
              );
              // 如果无法获取订单详情，使用预估值
              logger.warn(
                `使用预估值继续: 数量=${closeSize}, 价格=${currentPrice}`
              );
              ...
              logger.warn(
                `获取平仓订单详情失败，${retryCount}/${maxRetries} 次重试...`
              );
              await new Promise((resolve) => setTimeout(resolve, 300));
            }
          }
